const PATCH_VERSION = "3.2.0-live-visual";
const viewState = new WeakMap();

if (!document.querySelector('link[data-jbs-recovery-style]')) {
  const style = document.createElement("link");
  style.rel = "stylesheet";
  style.href = "/recovery-patch.css?v=3.2.0";
  style.dataset.jbsRecoveryStyle = "true";
...
function renderKey(mode) {
  return [
    ...
    num("stairWidth",4),
    val("shape","rectangle"),
    val("material","pressure_treated"),
    val("color","Natural"),
    val("railing","open_sides"),
    val("railStyle","Wood rail"),
    val("stairLocation","front"),
  ].join("|");
}
...
function buildTopSvg(length, depth, height, shape, railing, stairs, deck, rail) {
  ...
  const stairWidth = Math.min(65, w*.3);
  if (railing !== "none") {
    if (stairs === "front") {
      rails += `<line x1="${x0}" y1="${y0+h}" x2="${x0+w/2-stairWidth/2}" y2="${y0+h}" ${stroke}/><line x1="${x0+w/2+stairWidth/2}" y1="${y0+h}" x2="${x0+w}" y2="${y0+h}" ${stroke}/>`;
    } else rails += `<line x1="${x0}" y1="${y0+h}" x2="${x0+w}" y2="${y0+h}" ${stroke}/>`;
  }
  if (["open_sides","full"].includes(railing)) {
    const sideTop = y0+h/2-stairWidth/2, sideBottom = y0+h/2+stairWidth/2;
    rails += stairs === "left"
      ? `<line x1="${x0}" y1="${y0}" x2="${x0}" y2="${sideTop}" ${stroke}/><line x1="${x0}" y1="${sideBottom}" x2="${x0}" y2="${y0+h}" ${stroke}/>`
      : `<line x1="${x0}" y1="${y0}" x2="${x0}" y2="${y0+h}" ${stroke}/>`;
    rails += stairs === "right"
      ? `<line x1="${x0+w}" y1="${y0}" x2="${x0+w}" y2="${sideTop}" ${stroke}/><line x1="${x0+w}" y1="${sideBottom}" x2="${x0+w}" y2="${y0+h}" ${stroke}/>`
      : `<line x1="${x0+w}" y1="${y0}" x2="${x0+w}" y2="${y0+h}" ${stroke}/>`;
  }
  if (railing === "full") rails += `<line x1="${x0}" y1="${y0}" x2="${x0+w}" y2="${y0}" ${stroke}/>`;
  let stair = "";
  if (stairs !== "none" && height > .5) {
    const sw = stairWidth, steps = Math.max(2, Math.min(5, Math.ceil(height*12/7.5)));
    if (stairs === "front") {
...
function buildPerspectiveSvg(length, depth, height, shape, railing, stairs, deck, rail, mode) {
  ...
  if(railing!=="none") {
    if(stairs==="front") { rails+=railEdge([0,D],[L/2-sw/2,D]); rails+=railEdge([L/2+sw/2,D],[L,D]); }
    else rails+=railEdge([0,D],[L,D]);
  }
  if(["open_sides","full"].includes(railing)) {
    if(stairs==="left") { rails+=railEdge([0,0],[0,D*.58-sw/2]); rails+=railEdge([0,D*.58+sw/2],[0,D]); }
    else rails+=railEdge([0,0],[0,D]);
    if(stairs==="right") { rails+=railEdge([L,0],[L,D*.58-sw/2]); rails+=railEdge([L,D*.58+sw/2],[L,D]); }
    else rails+=railEdge([L,0],[L,D]);
  }
...
function enhanceBuilder(panel) {
  ...
  if(box.dataset.recoveryRenderKey!==key) {
    box.dataset.recoveryRenderKey=key;
...
document.addEventListener("click", (event) => {
  ...
  viewState.set(box,button.dataset.recoveryView||"front");
  delete box.dataset.recoveryRenderKey;
  enhanceBuilder(box.closest(".jbs-builder-preview-panel"));
}, true);
