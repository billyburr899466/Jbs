...
async function context(owner=false,width=390){
 ...
 if(owner)await ctx.addInitScript(({token,user})=>localStorage.setItem('sb-bjmdtxxlzzhxskxkllvt-auth-token',JSON.stringify({access_token:token,refresh_token:'test-refresh',token_type:'bearer',expires_in:3600,expires_at:Math.floor(Date.now()/1000)+3600,user})),{token,user});
...
try{await ready();browser=await playwright.launch({args:chromium.args,executablePath:await chromium.executablePath(),headless:true});await demoFlow(390);await demoFlow(1365);await ownerFlow();const denied=await fetch(url+'/api/quote-response',{method:'POST',headers:{'content-type':'application/json'},body:'{}'});assert.equal(denied.status,401);console.log('PASS API: unauthenticated quote response rejected');console.log('JBs 2.2.0 verification complete');}catch(e){console.error(e);process.exitCode=1;}finally{await browser?.close();server.kill();}
