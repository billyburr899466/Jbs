...
// Capture review images locally when requested; deployment checks assert the UI directly.
const capture=(page,options)=>process.env.JBS_CAPTURE_SCREENSHOTS==='1'?page.screenshot({...options,animations:'disabled'}):Promise.resolve();
...
async function demoFlow(width){
 ...
 await capture(page,{path:`test-results/home-${width}.png`});
 ...
 await capture(page,{path:`test-results/deck-${width}.png`});
 ...
 await capture(page,{path:`test-results/paint-${width}.png`});
 ...
 await capture(page,{path:`test-results/projects-${width}.png`});
 ...
 await capture(page,{path:`test-results/messages-${width}.png`});
...
await capture(page,{path:'test-results/owner-schedule.png'});
